...
from backtest.utils.io import load_yaml
from backtest.utils.tz import align_ts_to_index, to_naive_local

logger = logging.getLogger("backtest.windowing.plan")
...
def infer_backtest_start_from_analysis_cfg(
    *, calendar: pd.DatetimeIndex, analysis_cfg_path: Path
    ...
    raw = load_yaml(analysis_cfg_path) or {}
    ...
    cutoff_ts = align_ts_to_index(pd.Timestamp(cutoff_raw), calendar)
...
def generate_walkforward_windows_from_cfg(
    *,
    ...
    bt = _field(cfg, "backtest")
    if bt is None:
        raise KeyError("Missing required config section: backtest")

    wf = _field(bt, "walkforward")
    if not bool(_field(wf, "enabled", False)):
        return [], {"enabled": False}

    train_mode = str(_field(wf, "train_mode")).strip().lower()
    ...
    initial_train_months = _parse_months(
        _field(wf, "initial_train_months"),
        name="backtest.walkforward.initial_train_months",
    )
    test_months = _parse_months(
        _field(wf, "test_months"), name="backtest.walkforward.test_months"
    )
    step_months = _parse_months(
        _field(wf, "step_months"), name="backtest.walkforward.step_months"
    )
    ...
    r = _field(bt, "range")
    start_raw = _field(r, "start")
    end_raw = _field(r, "end")
    analysis_cfg_raw = _field(r, "analysis_cfg_path")
    analysis_cfg_path = (
