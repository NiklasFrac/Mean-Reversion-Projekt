...
from .params import cfg_with_markov_overrides


def _estimate_beta_ols(y: pd.Series, x: pd.Series) -> tuple[float | None, str | None]:
    return _pair_analysis.estimate_beta_ols_with_intercept_details(y, x)
...
def _simulate_stage_pnl(
    *,
    ...
    cfg_local = cfg_with_markov_overrides(cfg, markov_overrides)
    ...
    *,
    ...
    for pair in sorted((per_pair_prices or {}).keys(), key=str):
        ...
        if eval_dates is not None:
            ...
            history_idx = _pair_analysis.prior_train_history(
                train_idx, eval_index=eval_idx
            )
            allowed_idx = history_idx.union(eval_idx).sort_values()
            z_allowed = _pair_analysis.rolling_zscore_on_allowed_dates(
                spread,
