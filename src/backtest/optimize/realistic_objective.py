...
from backtest.config.cfg import parse_config
...
from .cv import _aggregate_scores, _build_cv_folds, _split_consecutive_indices
from .params import (
    apply_bo_params_to_cfg,
    candidate_bo_payload,
)
from .search import BAD_SCORE, _log_trial
from backtest.config.types import BOCVConfig

logger = logging.getLogger("backtest.optimize.realistic_objective")
...
def _score_realistic_segment(
    *,
    ...
    try:
        ...
        cfg_local = apply_bo_params_to_cfg(dict(cfg), candidate_bo_payload(theta))
        ...
        cfg_obj = parse_config(dict(cfg_local))
        borrow_ctx = build_borrow_context(cfg_obj.borrow)
        strat = build_strategy_from_cfg(dict(cfg_local))
        ...
        eq = pd.to_numeric(equity_from_stats(stats), errors="coerce").dropna()
        if eq.empty:
