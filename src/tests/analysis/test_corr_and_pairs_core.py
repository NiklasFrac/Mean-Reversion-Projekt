...
def _make_prices(seed=42, n=300):
    ...
    ra = rng.normal(0, 0.01, n)  # returns of A
    rb = ra + rng.normal(0, 0.002, n)  # returns of B ~ strongly correlated with A
    a = 100.0 * np.exp(np.cumsum(ra))
    b = 50.0 * np.exp(np.cumsum(rb))
    c = 75.0 * np.exp(np.cumsum(rng.normal(0, 0.01, n)))  # independent reference ticker
...
def test_shrink_corr_invariants_and_bounds():
    ...
    # diagonal ~1, symmetry, values in [-1, 1]
