...
def test_index_duplicates_handled(make_prices_file, tmp_path: Path):
    ...
    cfg = {
        ...
        "data_analysis": {
            ...
            "pct_thr1": 0.30,
            "pct_thr2": 0.35,
            "pair_pct_threshold": 0.8,  # as a fraction (0..1)
            ...
            # no train_cutoff_utc, we only test duplicate handling
        },
    ...
    # 1) Run succeeds, output exists
    assert isinstance(df_selected, pd.DataFrame)
    assert out_path.exists()
    # 2) Meta contains timings/env (observability), which indirectly shows the pipeline completed
    ...
    # 3) Pair A-B should appear because of the identical structure
