...
def test_scale_invariance(tmp_path: Path):
    ...
    # Erwartung: Paar (A,B) wird in beiden Läufen selektiert (Skalierung ändert Log-Returns nicht)
    assert any(p in pairs1 for p in ("A-B", "B-A"))
    assert any(p in pairs2 for p in ("A-B", "B-A"))

    # Optional: Invarianz (gleiche Pärchenmenge) – toleranter Check
    # (Bei deterministischem Seed & identischem Input sollte es identisch sein.)
    assert pairs1 == pairs2

    # Meta vorhanden (Observability)
