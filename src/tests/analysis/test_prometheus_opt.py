...
def test_prometheus_opt_out_noop():
    ...
    init_prometheus(cfg)  # should simply do nothing
    prom_observe({"stageA": 0.01}, 3)  # no-op, no error


def test_prometheus_opt_in_but_missing_client(monkeypatch):
    # simulate "no prometheus_client"
    ...
    init_prometheus(cfg)  # no-op, no error
    prom_observe({"stageA": 0.02, "stageB": 0.03}, 5)  # no error
    # _PROM stays empty
